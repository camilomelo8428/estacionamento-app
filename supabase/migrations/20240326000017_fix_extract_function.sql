...
-- Criar função para ocupar vaga de forma atômica com verificações melhoradas
...
DECLARE
  ...
  v_mensalidades_pendentes INT;
  v_ultima_mensalidade_paga DATE;
  v_dias_atraso INT;
BEGIN
  ...
  -- Se for ticket mensalista, fazer verificações detalhadas
  IF p_tipo = 'MENSALISTA' THEN
    -- Buscar ID do mensalista pela placa do veículo
    SELECT m.id INTO v_mensalista_id
    FROM mensalistas m
    INNER JOIN veiculos_mensalistas v ON v.mensalista_id = m.id
    ...
    -- Verificar quantidade de mensalidades pendentes
    SELECT COUNT(*) INTO v_mensalidades_pendentes
    FROM mensalidades 
    WHERE mensalista_id = v_mensalista_id
    AND status = 'PENDENTE';

    IF v_mensalidades_pendentes > 0 THEN
      RAISE EXCEPTION 'Mensalista possui % mensalidade(s) pendente(s) de pagamento', v_mensalidades_pendentes 
      USING ERRCODE = 'P0001';
    END IF;

    -- Verificar última mensalidade paga
    SELECT MAX(data_vencimento) INTO v_ultima_mensalidade_paga
    FROM mensalidades
    WHERE mensalista_id = v_mensalista_id
    AND status = 'PAGO';

    IF v_ultima_mensalidade_paga IS NULL THEN
      RAISE EXCEPTION 'Mensalista não possui nenhuma mensalidade paga' USING ERRCODE = 'P0001';
    END IF;

    -- Calcular dias de atraso (corrigido)
    v_dias_atraso := (CURRENT_DATE - v_ultima_mensalidade_paga);
    
    IF v_dias_atraso > 0 THEN
      RAISE EXCEPTION 'Mensalidade vencida há % dias. Por favor, regularize o pagamento', v_dias_atraso 
      USING ERRCODE = 'P0001';
    END IF;

    -- Verificar se tem mensalidade ativa para o mês atual
    SELECT EXISTS (
      SELECT 1 
      FROM mensalidades 
      WHERE mensalista_id = v_mensalista_id
    ...
    IF NOT v_tem_mensalidade_ativa THEN
      RAISE EXCEPTION 'Mensalista não possui mensalidade ativa para o mês atual' USING ERRCODE = 'P0001';
    END IF;
  END IF;
